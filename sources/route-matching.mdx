...
This is stricter than a pass-everything proxy. DataHarbor enforces that every request fits within the boundaries defined by your enrolled source.

## How it works

Every HTTP source has a **path template** set during enrollment. DataHarbor uses this template to:
...
Any request that fails admissibility is rejected **before** the upstream call is made. DataHarbor never proxies an ungoverned request.
