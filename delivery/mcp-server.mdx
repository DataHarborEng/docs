...
| Tool | Description |
|------|-------------|
| `describe_schema` | Describe the governed lease-visible surface for the current Virtual API, including route context, named visible fields, derived fields, and geofence restrictions |
...
- It describes the governed lease surface, not the full backing API schema.
- Redacted field names may still appear when the lease explicitly exposes them.
- Deleted fields are omitted.
- It does not expose Broker explored-model detail or hidden annotations.
...
Claude: I'll inspect the governed lease surface first.
[Calling describe_schema]

This Virtual API exposes customer records through `/v1/customers/{customerId}`.
Visible fields include `email` (redacted) and `full_name` (derived, hashed).
