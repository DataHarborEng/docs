...
<Info>
  Output formatting runs **after** [input normalization](./input-normalization) and the ordered controls pipeline. It only changes how the final governed payload is rendered. See [Data Pipeline](../core/data-pipeline) for the full request flow.
</Info>
...
<Tip>
  `input_format` is a separate setting used during [input normalization](./input-normalization). It affects how upstream data is parsed, not how the response is rendered.
</Tip>
