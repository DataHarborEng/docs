...
If the Virtual API Configuration specifies `output_format`, that always takes precedence over the URL extension. See [Output Formatting](/delivery/output-formatting) for details.
...
5. **Format** — Convert output if `output_format` is configured or a URL extension is present
