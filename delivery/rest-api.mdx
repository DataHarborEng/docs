...
If the Virtual API Configuration specifies `default_output_format`, that provides a default which can be overridden per-request via URL extension. See [Output Formatting](/delivery/output-formatting) for details.
...
4. **Apply controls** — Run the ordered control pipeline (redact, tokenize, transform)
5. **Format** — Convert output if `default_output_format` is configured or a URL extension is present
6. **Respond** — Return governed data
