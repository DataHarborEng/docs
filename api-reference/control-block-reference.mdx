...
| Path syntax | Use dot notation, with array segments like `"[contacts].email"`. Slash notation is not supported in v0.3. |
...
| `required` | All controls | No | When `true`, missing referenced fields fail instead of being skipped. For `coalesce`, all fields missing or empty also fails. |
...
**Behavior:**
- Same input → same token within the same matched object definition
...
Replace with one-way SHA-256 digest. Deterministic within the same matched object definition — same input always produces the same hash in the same scope.
...
`coalesce` treats missing values, `null`, and empty strings as empty. When you use nested paths, every entry in `fields` plus `into` must share the same parent path. If `into` already exists, the transform fails instead of overwriting it.
