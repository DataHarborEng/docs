...
- Set `required: true` when missing inputs should fail the request. For `coalesce`, missing, `null`, and empty-string values are treated as empty.
- `remove_source: true` works for both `combine` and `coalesce`.
- If the matched payload is an array, transforms run against each matching object element while preserving top-to-bottom control order.
