...
<Info>
  `redact` keeps the field in the response and only changes its value. If you want the field to disappear entirely, use `delete` from [Data Transform](/control-blocks/data-transform).
</Info>
...
- Data Control uses dot notation. Slash notation like `address/city` is rejected.
...
- Set `required: true` on a control when a missing field should fail instead of being skipped. That check is full-path strict: if any intermediate parent segment or the final field is missing, the request fails.
